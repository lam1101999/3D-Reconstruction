...
def loss_n(np, n, norm='L1', fc_p=None, fc=None):
    if norm == 'L1':
        ...
        cosine = torch.nn.functional.cosine_similarity(np, n, dim=1)
        loss = (1-cosine)
        loss = loss.mean()
...
def discriminator_loss(real_vertex_logits, real_facet_logits, fake_vertex_logits, fake_facet_logits,device):
    ...
    # vertex loss
    target_real_vertex = torch.FloatTensor(real_vertex_logits.size()).uniform_(0.99,1.0).to(device)
    target_fake_vertex = torch.FloatTensor(real_vertex_logits.size()).uniform_(0.0, 0.01).to(device)
    ...
    # facet loss
    target_real_facet = torch.FloatTensor(real_facet_logits.size()).uniform_(0.99, 1.0).to(device)
    target_fake_facet = torch.FloatTensor(real_facet_logits.size()).uniform_(0.0, 0.01).to(device)
...
def generator_loss(fake_vertex_logits, fake_facet_logits, device):
    ...
    # vertex loss
    target_real_vertex = torch.FloatTensor(fake_vertex_logits.size()).uniform_(0.99, 1.0).to(device)
    vertex_loss = (fake_vertex_logits - target_real_vertex).pow(2).sum(1).mean()
    
    # facet loss
    target_real_facet = torch.FloatTensor(fake_facet_logits.size()).uniform_(0.99, 1.0).to(device)
