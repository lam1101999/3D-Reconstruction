...
def predict_one(opt, net, device, filename, rst_filename=None, filename_gt=None):
    ...
    all_data = DualDataset.process_one_data(
        filename, opt.sub_size, filename_gt)
    centroid = all_data[0][0][0].centroid
    scale = all_data[0][0][0].scale
    print(
        F"---------- time: {(datetime.now()-time_start).total_seconds():7.4f} s")
    ...
    om.write_mesh(F"{rst_filename[:-4]}-60.obj",
                  om.TriMesh(V.numpy(), mesh_noisy.fv_indices()))

    print(
        F"---------- time: {(datetime.now()-time_start1).total_seconds():7.4f} s")
    ...
    print(
        F"angle1: {angle1:9.6f},  angle2: {angle2:9.6f},  faces: {Np.shape[0]:>6},  time: {(datetime.now()-time_start).total_seconds():7.4f} s,  '{os.path.basename(rst_filename)}'")
...
def predict_dir(params_path, data_dir=None, sub_size=None, gpu=-1):
    ...
    if data_dir is None:
        ...
        original_dir = os.path.join(
            DATASET_DIR, opt.data_type, 'test', 'original')
    ...
    import network
    net = network.DualGNN(force_depth=opt.force_depth,
    ...
    for i, noisy_file in enumerate(filenames):
        ...
        #     continue
        angle1, angle2, count = predict_one(opt, net, device, noisy_file, rst_file, None if len(
...
if __name__ == "__main__":
    ...
    if IS_DEBUG:
        parser.add_argument('--params_path', type=str, default=r"E:\thesis\3D-Reconstruction\src\train\log\GeoBi-GNN_Synthetic_2024-01-09-23-08-23\GeoBi-GNN_Synthetic_params.pth", help='params_path')
    ...
    predict_dir(opt.params_path, data_dir=opt.data_dir,
                sub_size=opt.sub_size, gpu=opt.gpu)
