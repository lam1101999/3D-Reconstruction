...
class DualDataset(torch_geometric.data.Dataset):
    ...
    def process_data(self):
        ...
        # bar = "{l_bar}{bar}{r_bar}"
        if os.path.exists(self.processed_information_file_path): # Warning Edit here
    ...
    def process_one_data(noisyfile, submesh_size, originalfile=None, obj=None):
        ...
        _, centroid, scale = data_util_geobi.center_and_scale(points_noisy, mesh_noisy.ev_indices())
        ...
        if mesh_noisy.n_faces() <= submesh_size:
            ...
            for sub_num in range(1, sys.maxsize):
                ...
                select_faces = data_util_geobi.mesh_get_neighbor_np(fv_indices, vf_indices, seed, neighbor_count=submesh_size)
                ...
                if len(select_faces) > filter_patch_count:
                    ...
                    if obj is None or not os.path.exists(pro_name):
                        ...
                        V_idx, F = data_util_geobi.get_submesh(fv_indices, select_faces)
    ...
    def process_one_submesh(mesh_n, name='graph', mesh_o=None):
        ...
        edge_dual_fv = data_util_geobi.build_edge_fv(fv_indices)
        ...
        # edge_idx_v = data_util_geobi.build_vertex_graph(ev_indices, vv_indices)  # undirected grah, with self_loops
        edge_wei_v = data_util_geobi.calc_weight(pos_v, normal_v, edge_idx_v)
        ...
        edge_idx_f = data_util_geobi.build_facet_graph(fv_indices, vf_indices)  # undirected grah, with self_loops
        edge_wei_f = data_util_geobi.calc_weight(pos_f, normal_f, edge_idx_f)
        ...
        # _, centroid, scale = data_util_geobi.center_and_scale(pos_v, ev_indices, 0)
...
def process_GT_Kinect_Fusion(noisy_dir, original_dir, filtered_dir):
    ...
    for _, (noisyfile, originalfile, filteredfile) in enumerate(zip(noisy_files, original_files, filtered_files)):
        ...
        for i in range(normal1.shape[0]):
            faces_ring2 = data_util_geobi.mesh_get_neighbor_np(fv_indices, vf_indices, i, ring_count=2)
